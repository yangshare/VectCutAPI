...
def build_asset_path(draft_folder, draft_id, asset_type, material_name):
...
def save_draft_background(draft_id, draft_folder, task_id):
    ...
    try:
        ...
        # Save draft
        current_dir = os.path.dirname(os.path.abspath(__file__))  # vectcut/features/draft/
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))  # 项目根（文件深 3 级）
        ...
        template_source_dir = os.path.join(project_root, template_dir)
        ...
        if load_config().is_upload_draft:
